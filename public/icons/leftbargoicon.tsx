import type { SVGProps } from "react";

export default function LeftbarGoIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none" {...props}>
      <path d="M6.78305 5.79323L14.5655 3.19906C18.058 2.0349 19.9555 3.94156 18.8005 7.43406L16.2064 15.2166C14.4647 20.4507 11.6047 20.4507 9.86305 15.2166L9.09305 12.9066L6.78305 12.1366C1.54888 10.3949 1.54888 7.54406 6.78305 5.79323Z" stroke="currentColor" strokeWidth="var(--left-icon-stroke, 1.5)" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M9.26758 12.5125L12.5492 9.22168" stroke="currentColor" strokeWidth="var(--left-icon-stroke, 1.5)" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}


